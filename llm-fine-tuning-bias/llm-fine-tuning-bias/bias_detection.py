...
class BiasDetector:
    ...
    def analyze_bias(self, attributions_batch, group_a_tokens, group_b_tokens):
        ...
        return {
            ...
            "wasserstein_distance": float(stats.wasserstein_distance(masses_a, masses_b)),
            "jensen_shannon_divergence": float(spatial.distance.jensenshannon(masses_a, masses_b)) if (len(masses_a) > 0 and len(masses_b) > 0 and np.sum(masses_a) > 0 and np.sum(masses_b) > 0) else 0.0
        }

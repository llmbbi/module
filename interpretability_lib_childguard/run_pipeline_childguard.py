#!/usr/bin/env python3
"""
ChildGuard Interpretability Pipeline

Adapted from run_pipeline_modular.py for ChildGuard dataset.
"""
...
import pandas as pd # Import pandas if available, otherwise use csv (or rely on datasets which handles csv)
...
def load_childguard_data(data_path, test_size=0.1, val_size=0.1, seed=42):
    """
    Load ChildGuard dataset from CSV and split into train/val/test.
    ChildGuard CSV columns: text, actual_class, predicted_class, Age_Group
    ...
    if "actual_class" in dataset.column_names:
        dataset = dataset.rename_column("actual_class", "label")
    
    # Filter out necessary columns only
    ...
    # The val_size is relative to the original size, so we need to adjust
    # If val_size is 0.1 of total, and test_size is 0.1, then train is 0.8.
    # From the remaining 0.9, we need 0.1/0.9 fraction for validation.
    adjusted_val_size = val_size / (1 - test_size)
    
    train_val = train_val_dataset.train_test_split(test_size=adjusted_val_size, seed=seed)
...
def detect_bias_with_attributions(attributor, eval_data, sample_size=100):
    ...
    relevant_indices = [i for i, item in enumerate(eval_data) 
                        if any(t in str(item["sentence"]).lower().split() for t in all_demographic_tokens)]
    ...
    sample_texts = [str(eval_data[i]["sentence"]) for i in selected_indices]
...
def main():
    ...
    # Load dataset
    dataset_splits = load_childguard_data(args.data_path)
    train_data = dataset_splits["train"]
    val_data = dataset_splits["validation"]
    test_data = dataset_splits["test"]

    # Use validation set for 'eval_data' in XAI and general evaluation, 
    # unless we specifically want to evaluate on test set. 
    # For this pipeline, let's use validation for monitoring as usual, but we could use test.
    # We will use 'val_data' to be consistent with fine-tuning validation.
    # Wait, best practice is to test on test set. Let's use test_data for final evaluation.
    # But during fine-tuning we pass train_data as 'train_dataset'. Unsloth's SFTTrainer can split automatically if not provided,
    # but we can provide 'eval_dataset'.
    
    eval_data = test_data # Use test set for XAI and Performance metrics reporting
